...
contract('WyvernExchange', (accounts) => {
  ...
  it('should correctly hash order', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[0], staticTarget: ZERO_ADDRESS, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '0', salt: '0'}
  ...
  it('should validate valid order parameters', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[0], staticTarget: exchange.inst.address, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '1000000000000', salt: '0'}
  ...
  it('should not validate order parameters with invalid staticTarget', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[0], staticTarget: ZERO_ADDRESS, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '1000000000000', salt: '0'}
        return exchange.validateOrderParameters(example).then(valid => {
  ...
  it('should not validate order parameters with listingTime after now', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[0], staticTarget: exchange.inst.address, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '1000000000000', expirationTime: '1000000000000', salt: '0'}
        return exchange.validateOrderParameters(example).then(valid => {
          assert.equal(false, valid, 'Should not have validated')
        })
      })
  ...
  it('should not validate order parameters with expirationTime before now', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[0], staticTarget: exchange.inst.address, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '0', salt: '0'}
        return exchange.validateOrderParameters(example).then(valid => {
          assert.equal(false, valid, 'Should not have validated')
        })
      })
  ...
  it('should validate valid authorization by signature', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[1], staticTarget: exchange.inst.address, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '1000000000000', salt: '100230'}
  ...
  it('should not allow approval twice', () => {
    return withExchangeAndRegistry()
      .then(({exchange, registry}) => {
        const example = {registry: registry.address, maker: accounts[1], staticTarget: exchange.inst.address, staticSelector: '0x00000000', staticExtradata: '0x', maximumFill: '1', listingTime: '0', expirationTime: '1000000000000', salt: '1010'}
